...
import { Row, Col ,Table,Form,Input, Button,Card,Select,DatePicker,Icon,Radio } from 'antd';
import BreadcrumbCustom from '../BreadcrumbCustom';
import styles from '../query/TableList.less';
import moment from 'moment';
import { incomeStation,downLoad } from '../../axios';
const RadioGroup = Radio.Group;
const RangePicker = DatePicker.RangePicker;
const FormItem = Form.Item;
const Option = Select.Option;

const dotManageColumns = [{
        title: '所属城市',
        dataIndex: 'city',
      },{
        title: '区域负责人',
...
class dotManage extends React.Component {
  constructor(props) {
    super(props);
    this.state={
      date:[],
  ...
  handleSearch = (e) => {
   e?e.preventDefault():null;
    this.props.form.validateFields((err,values) => {
      ...
      incomeStation([values.city,values.DistrictHead,values.Dot,values.displayStyle?values.displayStyle:0,beginTime,endTime]).then(res => {
        let i = 0;
      ...
      this.props.form.validateFields((err, values) => {
        ...
        downLoad([]).then(res => {
           if(res.data&&res.data.status===1){
            ...
            let url='http://manage.dianxiakg.com/api/income/site/download/'+res.data.id+'?beginTime='+beginTime+'&endTime='+endTime+'&showStyle='+style;
            if(values.city){
  ...
  render() {
    ...
    return (
      ...
      <BreadcrumbCustom first="营收统计" second="网点营收统计" />
        <div className="gutter-box">
            <Card bordered={false} title="网点营收统计查询" className={'no-padding'}>
            ...
            <Col md={5} sm={24} offset={2}>
                    ...
                    <Button style={{ marginTop: '20px' }} onClick={this.download}><Icon type="download" />导出表格</Button>
           ...
           <Row>
            <Col md={20} offset={2}>
             <p>查询结果：共<span style={{color:'#49a9ee'}}>{this.state.date.length}</span>个网点营收记录</p>
            <Table columns={dotManageColumns} dataSource={this.state.date} bordered />
            </Col>
          </Row>
          </Card>
        </div>
...
const DotManage = Form.create()(dotManage);
export default connect(mapStateToProps)(DotManage);
